...
const SAMPLE_META_TAGS = [
    {
        ...
        meta_description: 'Boost your website rankings with AI-powered SEO analysis, keyword research, and content optimization. Start your free trial today.',
    ...
    },
    {
        ...
        meta_description: 'Learn everything about SEO in 2025. From technical optimization to content strategy, this comprehensive guide covers it all.',
...
async function seedDatabase() {
    ...
    try {
        ...
        await client.query(`DELETE FROM api_usage WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%@prismify.test')`);
        await client.query(`DELETE FROM meta_tags WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%@prismify.test')`);
        await client.query(`DELETE FROM seo_analyses WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%@prismify.test')`);
        await client.query(`DELETE FROM seo_projects WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%@prismify.test')`);
        await client.query(`DELETE FROM api_keys WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%@prismify.test')`);
        await client.query(`DELETE FROM subscription_history WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%@prismify.test')`);
        ...
        for (const user of TEST_USERS) {
            const result = await client.query(
                ...
                [user.email, hashPassword(user.password), user.full_name, user.company_name, user.subscription_tier, user.subscription_status, user.stripe_customer_id]
            );
        ...
        for (const user of userIds) {
            ...
            await client.query(
                `INSERT INTO api_keys (user_id, name, key_hash, key_prefix, is_active)
                 VALUES ($1, $2, $3, $4, true)`,
                [user.id, `${user.tier.charAt(0).toUpperCase() + user.tier.slice(1)} API Key`, keyHash, keyPrefix]
            );
        ...
        for (const user of userIds) {
            ...
            await client.query(
                ...
                [
                    user.id,
                    user.tier,
                    tierPrices[user.tier],
                    `sub_test_${user.tier}_001`
                ]
            );
        }
